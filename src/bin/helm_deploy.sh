...
function install() {
  ...
  kubectl annotate pdb -n "${HELM_NAMESPACE}" "${HELM_RELEASE}"-wordpress-php meta.helm.sh/release-name="${HELM_RELEASE}"
  kubectl annotate pdb -n "${HELM_NAMESPACE}" "${HELM_RELEASE}"-wordpress-php meta.helm.sh/release-namespace="${HELM_NAMESPACE}"
  kubectl label pdb -n "${HELM_NAMESPACE}" "${HELM_RELEASE}"-wordpress-php app.kubernetes.io/managed-by=Helm

  kubectl annotate pdb -n "${HELM_NAMESPACE}" "${HELM_RELEASE}"-wordpress-openresty meta.helm.sh/release-name="${HELM_RELEASE}"
  kubectl annotate pdb -n "${HELM_NAMESPACE}" "${HELM_RELEASE}"-wordpress-openresty meta.helm.sh/release-namespace="${HELM_NAMESPACE}"
  kubectl label pdb -n "${HELM_NAMESPACE}" "${HELM_RELEASE}"-wordpress-openresty app.kubernetes.io/managed-by=Helm
